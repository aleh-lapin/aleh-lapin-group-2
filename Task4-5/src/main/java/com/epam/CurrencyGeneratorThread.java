...
public class CurrencyGeneratorThread extends Thread{
	...
	private void changeRate() {
		...
		Long currencyId = Long.valueOf(random.nextInt(Currency.values().length));
		Long currencyIdTo = Long.valueOf(random.nextInt(Currency.values().length));
		while (currencyId.equals(currencyIdTo)) {
			currencyIdTo = Long.valueOf(random.nextInt(Currency.values().length));
		}
		CurrencyRate currencyRate = iCurrencyRate.findCurrencyRate(currencyId);
		double newRate = getGaussian(random, currencyRate.getToCurrencyRate().get(currencyIdTo));
	...
	private void initUSD() {
		...
		CurrencyRate currencyRate = new CurrencyRate(Currency.USD.getId(), toCurrencyRate);
		iCurrencyRate.addOrUpdateCurrencyRate(currencyRate);
	}
	...
	private void initEUR() {
		...
		CurrencyRate currencyRate = new CurrencyRate(Currency.EUR.getId(), toCurrencyRate);
		iCurrencyRate.addOrUpdateCurrencyRate(currencyRate);
	}
	...
	private void initRU() {
		...
		CurrencyRate currencyRate = new CurrencyRate(Currency.RU.getId(), toCurrencyRate);
		iCurrencyRate.addOrUpdateCurrencyRate(currencyRate);
	}
